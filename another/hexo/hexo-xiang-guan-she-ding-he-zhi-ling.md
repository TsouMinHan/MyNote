...
接著打開`_config.yml`，theme改成next就可以了，記得要在重新佈署一次唷! 我記得是因為靜態檔案的關係，佈署之後網頁不會馬上更新，建議使用Ctrl+F5更新頁面，不同瀏覽器似乎有不同的方式，若有問題的話再查一下吧!
...
預設是Muse，可以到[這邊](https://github.com/theme-next/hexo-theme-next)查看樣式，另外如果佈署之後發現沒有變化的話輸入`hexo d`清除之前建立的靜態檔案，然後再重新佈署一次，因為css檔案的問題。
...
到[Disqus](https://disqus.com/)點Get Started，接著註冊帳號\(我是用google的\)，接著點選I want to install Disqus on my site。 要進行些設定，Website Name 就是自定的名稱，會顯示在，會顯示在紅色箭頭處。 另外可以選擇語言，有中文的，但是我看參考來源他的留言板是簡體中文的，所以我選擇英文。
